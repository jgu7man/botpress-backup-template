...
const _findRecord = async () => {
  ...
  try {
    if (!phone || !conversationId) {
      console.error(
        "⚠️ No phone or conversation ID provided. Cannot find record."
      );
      return null;
    }
    ...
    if (phone) {
      ...
      console.log(
        "🔎 Searching for record by conversation ID:",
        conversationId
      );
      filter = { CONVERSATION_ID: conversationId };
    } else {
      console.warn(
        "❗️ No phone or conversation ID provided to search for record."
      );
      return null;
    }
    ...
    if (recordResults.length > 0) {
      ...
      workflow.pageId = lastRecord.NOTION_PAGE_ID || "";

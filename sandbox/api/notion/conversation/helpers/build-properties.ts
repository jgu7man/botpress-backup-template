...
class NotionPropertiesBuilder {
   ...
   */
  build(properties: Array<{ id: string; value: string; type: string }>) {
    const result: Record<string, any> = {};

    properties.forEach(({ id, value, type }) => {
...
new NotionPropertiesBuilder().build(properties);

/**
 * Main function to build Notion properties from conversation properties
 */
export const buildNotionProperties = (
  properties: Array<{ id: string; value: string; type: string }>
) => {
  return new NotionPropertiesBuilder().build(properties);
};
